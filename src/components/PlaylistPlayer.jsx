...
function PlaylistPlayer({ playlistData }) {
    ...
    const [currChannelName, setCurrChannelName] = useState(
        "Create a playlist of content archived on Patchwork! Enqueue videos by providing their URLs or IDs",
    );
    ...
    useEffect(() => {
        if (playlistData) {
            ...
            const firstVideoUrlMatch = videos[0].match(
                /\/([a-zA-Z0-9_-]+)\.webm$/,
            );
            const firstVideoId = firstVideoUrlMatch
                ? firstVideoUrlMatch[1]
                : "Unknown";
    ...
    const handleVideoChange = (index) => {
        ...
        const videoUrlMatch = playlist[index].match(
            /\/([a-zA-Z0-9_-]+)\.webm$/,
        );
    ...
    const handleRemoveVideo = (videoIndex) => {
        ...
        if (videoIndex <= currentVideoIndex && updatedPlaylist.length) {
            ...
            if (currentVideoIndex - 1 >= 0) {
                const videoUrlMatch = playlist[currentVideoIndex - 1].match(
                    /\/([a-zA-Z0-9_-]+)\.webm$/,
                );
                const videoId = videoUrlMatch ? videoUrlMatch[1] : "Unknown";
    ...
    const handleVideoEnd = () => {
        ...
        if (nextVideoIndex < playlist.length) {
            ...
            const videoUrlMatch = playlist[nextVideoIndex].match(
                /\/([a-zA-Z0-9_-]+)\.webm$/,
            );
            const videoId = videoUrlMatch ? videoUrlMatch[1] : "Unknown";
            setCurrVideoID(videoId);
    ...
    return (
        <>
            ...
            <div className="container mx-auto flex flex-col md:flex-row justify-center items-start space-y-4 md:space-y-0 md:space-x-4">
                ...
                <div
                    className="playlist-sidebar w-full md:w-1/3 bg-gray-200 p-4"
            ...
            <div className="px-12 info-box p-5 fixed md:px-48">
                <a href={"/watch?v=" + currVideoID}>
                    {" "}
                    <h1 className="text-2xl font-bold text-white mb-3 hover:underline">
                        {currTitle}
                    </h1>
                </a>
                <a href={"/channel/" + currChannelId}>
                    <p className="text-white text-xl mb-4 hover:underline">
                        {currChannelName}
                    </p>
                </a>
